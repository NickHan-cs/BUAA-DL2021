    ...
    def _glorot_init(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                init.xavier_uniform_(m.weight, init.calculate_gain('tanh'))  # TODO: gain?
...
class STDN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.lstm_seq_len = self.config.get('input_window', 7)
        self.feature_vec_len = self.data_feature.get('feature_vec_len', 160)
        self.nbhd_size = 2 * self.config.get('cnn_nbhd_size', 3) + 1
        ...
        self.output_shape = config.get('output_dim', 2)
    ...
    def _init_model(self):
        ...
        '''
        # 1st level gate
        self.nbhd_cnns_1st = [nn.Sequential(
            nn.Conv2d(in_channels=self.nbhd_type, out_channels=64, kernel_size=(3, 3), padding=2), # TODO: padding?
            nn.ReLU()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        self.flow_cnns_1st = [nn.Sequential(
            nn.Conv2d(in_channels=self.flow_type, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU(),
            nn.Sigmoid()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        ...
        # 2nd level gate
        self.nbhd_cnns_2nd = [nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        self.flow_cnns_2nd = [nn.Sequential(
            nn.Conv2d(in_channels=self.flow_type, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU(),
            nn.Sigmoid()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        ...
        # 3rd level gate
        self.nbhd_cnns_3rd = [nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        self.flow_cnns_3rd = [nn.Sequential(
            nn.Conv2d(in_channels=self.flow_type, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU(),
            nn.Sigmoid()
        ).to(self.device) for i in range(self.lstm_seq_len)]
        ...
        # attention part
        self.att_nbhd_cnns_1st = [[nn.Sequential(
            nn.Conv2d(in_channels=self.nbhd_type, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_cnns_1st = [[nn.Sequential(
            nn.Conv2d(in_channels=self.flow_type, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_gate_1st = [[nn.Sigmoid().to(self.device) for j in range(self.att_lstm_seq_len)]
                                  for i in range(self.att_lstm_num)]
        # [[nbhd * flow]] shape=[att_lstm_num, att_lstm_seq_len, (batch_size, 64, nbhd_size, nbhd_size)]

        self.att_nbhd_cnns_2nd = [[nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_cnns_2nd = [[nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_gate_2nd = [[nn.Sigmoid().to(self.device) for j in range(self.att_lstm_seq_len)]
                                  for i in range(self.att_lstm_num)]
        # [[nbhd * flow]] shape=[att_lstm_num, att_lstm_seq_len, (batch_size, 64, nbhd_size, nbhd_size)]

        self.att_nbhd_cnns_3rd = [[nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_cnns_3rd = [[nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=2),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        self.att_flow_gate_3rd = [[nn.Sigmoid().to(self.device) for j in range(self.att_lstm_seq_len)]
                                  for i in range(self.att_lstm_num)]
        # [[nbhd * flow]] shape=[att_lstm_num, att_lstm_seq_len, (batch_size, 64, nbhd_size, nbhd_size)]

        self.att_nbhd_vecs = [[nn.Sequential(
            nn.Flatten(),
            nn.Linear(64 * self.nbhd_size * self.nbhd_size, self.cnn_flat_size),
            nn.ReLU()
        ).to(self.device) for j in range(self.att_lstm_seq_len)] for i in range(self.att_lstm_num)]
        ...
        self.att_lstms = [nn.LSTM(input_size=self.feature_vec_len + self.cnn_flat_size, hidden_size=self.lstm_out_size,
        ...
        # compare
        self.att_low_level = [CBAAttention(self.device, self.lstm_out_size, self.lstm_out_size)
                              for i in range(self.att_lstm_num)]
        ...
        self.lstm_all = nn.Linear(self.lstm_out_size + self.lstm_out_size, self.output_shape).to(self.device)
    ...
    def forward(self, batch):
        ...
        # TODO: why this inputs but later convs
        nbhd_convs = [nbhd_convs[ts] * flow_convs[ts] for ts in range(self.lstm_seq_len)]
    ...
    def calculate_loss(self, batch):
        ...
        y_predicted = y_predicted.reshape(-1, 1, 10, 20, self.output_shape)
